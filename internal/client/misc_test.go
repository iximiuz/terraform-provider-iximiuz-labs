...
func TestGetAuthor(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/author" {
			t.Errorf("expected path /author, got %s", r.URL.Path)
		}
		w.Header().Set("Content-Type", "application/json")
		fmt.Fprint(w, `{"userId":"user-123","displayName":"Test Author","official":true}`)
	}))
	defer ts.Close()

	c := newTestClient(ts)
	author, err := c.GetAuthor(context.Background())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if author.UserID != "user-123" {
		t.Errorf("expected userID %q, got %q", "user-123", author.UserID)
	}
	if author.DisplayName != "Test Author" {
		t.Errorf("expected displayName %q, got %q", "Test Author", author.DisplayName)
	}
	if !author.Official {
...
func TestCreateAuthor(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if req.DisplayName != "New Author" {
			t.Errorf("expected displayName %q, got %q", "New Author", req.DisplayName)
		}

		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusCreated)
		fmt.Fprint(w, `{"userId":"user-new","displayName":"New Author"}`)
	}))
	defer ts.Close()

	c := newTestClient(ts)
	author, err := c.CreateAuthor(context.Background(), CreateAuthorRequest{
		DisplayName: "New Author",
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if author.DisplayName != "New Author" {
		t.Errorf("expected displayName %q, got %q", "New Author", author.DisplayName)
	}
}
